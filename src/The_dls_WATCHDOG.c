/******************************************************************************************************************************/
/* ABLS-AGENT-DLS/src/The_dls_WATCHDOG.c        Déclaration des fonctions pour la gestion des temporisations                            */
...
/******************************************************************************************************************************/
 gboolean Dls_data_WATCHDOG_set_from_thread_watchdog ( JsonNode *request )
  { if (!Json_has_member ( request, "agent_tech_id" ))
    { Info( __func__, "mqtt", "local", LOG_ERR, "SET_WATCHDOG: missing agent_tech_id" );  return(FALSE); }
   if (!Json_has_member ( request, "agent_acronyme" ) )
    { Info( __func__, "mqtt", "local", LOG_ERR, "SET_WATCHDOG: missing agent_acronyme" ); return(FALSE); }
   ...
   gchar *agent_acronyme = Json_get_string ( request, "agent_acronyme" );
    ...
    Info( __func__, "mqtt", "local", LOG_INFO, "SET_WATCHDOG for: '%s:%s'=%d",
          agent_tech_id, agent_acronyme, consigne );
   struct DLS_WATCHDOG *bit = Dls_data_WATCHDOG_lookup ( agent_tech_id, agent_acronyme );
